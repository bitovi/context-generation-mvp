...
jest.mock('../instruction-generator', () => ({ generateInstructions: jest.fn() }));

import { contextGenerationRunner } from '../runner';
...
describe('contextGenerationRunner', () => {
  beforeEach(() => {
    (loadConfig as jest.Mock).mockReturnValue({ repo: 'r', featureBranch: 'f', baseBranch: 'b' });
  ...
  test('runs checkout and generateInstructions', async () => {
    ...
    expect(checkoutRepo).toHaveBeenCalledTimes(2);
